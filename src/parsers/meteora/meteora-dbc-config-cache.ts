...
export interface MeteoraDBCConfigData {
  ...
  quoteMint: string;
  migrationQuoteThreshold: bigint; // totalQuoteFundRaising in lamports
...
class MeteoraDBCConfigCacheClass {
  private cache: Map<string, MeteoraDBCConfigData> = new Map();
   ...
   */
  async fetchFromRpc(connection: Connection, configAddress: string): Promise<MeteoraDBCConfigData | undefined> {
    // Check cache first
   ...
   */
  async fetchMultipleFromRpc(
    connection: Connection,
    configAddresses: string[]
  ): Promise<Map<string, MeteoraDBCConfigData>> {
    ...
    try {
      const pubkeys = toFetch.map((addr) => new PublicKey(addr));
   ...
   */
  static decodePoolConfigAccount(
    configAddress: string,
    data: Buffer
  ): Omit<MeteoraDBCConfigData, 'createdAt' | 'lastAccessedAt'> | undefined {
    try {
      ...
      const quoteMint = base58.encode(data.subarray(QUOTE_MINT_OFFSET, QUOTE_MINT_OFFSET + 32));
...
export const MeteoraDBCConfigCache = new MeteoraDBCConfigCacheClass();
