...
import RaydiumLaunchpadIdl from '../idls/raydium-launchpad.json';
...
export class RaydiumLaunchpadDecoder extends IdlDecoder {
  constructor() {
    super(RaydiumLaunchpadIdl as Idl);
  }
  ...
  identifyEvent(data: Buffer | Uint8Array): string | null {
    if (!this.isRaydiumLCPEvent(data)) return null;
  ...
  decodePoolCreateEvent(data: Buffer | Uint8Array): RaydiumLCPPoolCreateEvent | null {
    ...
    const eventName = this.identifyEvent(buffer);
    if (eventName !== 'PoolCreateEvent') return null;
   ...
   */
  private parseTradeEventData(data: Buffer): RaydiumLCPTradeEvent {
    ...
    const poolState = base58.encode(data.subarray(offset, offset + 32));
    offset += 32;
    ...
    if (hasCreatorFee) offset += 8;

    const shareFee = data.readBigUInt64LE(offset);
    offset += 8;

    const tradeDirection = data.readUInt8(offset) as TradeDirection;
    offset += 1;

    const poolStatus = data.readUInt8(offset) as PoolStatus;
    offset += 1;
   ...
   */
  private parsePoolCreateEventData(data: Buffer): RaydiumLCPPoolCreateEvent {
    let offset = 0;
   ...
   */
  decodeAnyEvent(data: Buffer | Uint8Array): { type: string; data: RaydiumLCPEventData } | null {
...
export const raydiumLaunchpadDecoder = new RaydiumLaunchpadDecoder();
