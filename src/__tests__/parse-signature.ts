...
import { MemeEvent } from '../types';

const RPC_URL = process.env.RPC_URL || 'https://api.mainnet-beta.solana.com';

async function parseSignature(signature: string) {
  ...
  const result = parser.parseAll(tx as any, {
    tryUnknowDEX: true,
  ...
  if (result.trades.length > 0) {
    ...
    for (const trade of result.trades) {
      ...
      if (trade.user) console.log(`  User: ${trade.user}`);
      if (trade.Pool && trade.Pool.length > 0) console.log(`  Pool: ${trade.Pool.join(', ')}`);
    }
  }
  ...
  if (result.liquidities.length > 0) {
    ...
    for (const liq of result.liquidities) {
      ...
      if (liq.token0Mint) console.log(`  Token0: ${liq.token0Amount} ${liq.token0Mint}`);
      if (liq.token1Mint) console.log(`  Token1: ${liq.token1Amount} ${liq.token1Mint}`);
    }
  }
  ...
  if (result.memeEvents.length > 0) {
    ...
    for (const event of result.memeEvents) {
      ...
      if (event.poolAddress) console.log(`  Pool Address: ${event.poolAddress}`);
      ...
      if (event.type === 'CREATE' && event.baseToken) {
        ...
        if (event.baseToken.programId) console.log(`  Token Program: ${event.baseToken.programId}`);
        if (event.quoteToken) console.log(`  Quote Token: ${event.quoteToken.symbol} (${event.quoteToken.mint})`);
        if (event.creatorAddress) console.log(`  Creator: ${event.creatorAddress}`);
        if (event.curveType) console.log(`  Curve Type: ${event.curveType}`);
        if (event.curveQuoteReserves !== undefined) console.log(`  Curve Quote Reserves: ${event.curveQuoteReserves}`);
        if (event.curveBaseReserves !== undefined) console.log(`  Curve Base Reserves: ${event.curveBaseReserves}`);
        if (event.vaultQuoteReserves !== undefined) console.log(`  Vault Quote Reserves: ${event.vaultQuoteReserves}`);
        if (event.vaultBaseReserves !== undefined) console.log(`  Vault Base Reserves: ${event.vaultBaseReserves}`);
        if (event.graduationThreshold !== undefined) console.log(`  Graduation Threshold: ${event.graduationThreshold}`);
        if (event.initialSaleSupply !== undefined) console.log(`  Initial Sale Supply: ${event.initialSaleSupply}`);
        if (event.configAddress) console.log(`  Config Address: ${event.configAddress}`);
      }

      if (event.type === 'BUY' || event.type === 'SELL') {
        if (event.inputToken) console.log(`  Input: ${event.inputToken.amount} ${event.inputToken.mint}`);
        if (event.outputToken) console.log(`  Output: ${event.outputToken.amount} ${event.outputToken.mint}`);
        if (event.fee !== undefined) console.log(`  Fee: ${event.fee}`);
        if (event.vaultQuoteReserves !== undefined) console.log(`  Vault Quote Reserves: ${event.vaultQuoteReserves}`);
        if (event.vaultBaseReserves !== undefined) console.log(`  Vault Base Reserves: ${event.vaultBaseReserves}`);
        if (event.curveQuoteReserves !== undefined) console.log(`  Curve Quote Reserves: ${event.curveQuoteReserves}`);
        if (event.curveBaseReserves !== undefined) console.log(`  Curve Base Reserves: ${event.curveBaseReserves}`);
      ...
      if (event.type === 'COMPLETE' || event.type === 'MIGRATE') {
        ...
        if (event.migratedSolAmount !== undefined) console.log(`  Migrated SOL Amount: ${event.migratedSolAmount}`);
        if (event.migrationFee !== undefined) console.log(`  Migration Fee: ${event.migrationFee}`);
      }
    }
  }
  ...
  if (result.transfers.length > 0) {
    ...
    for (const transfer of result.transfers.slice(0, 10)) {
      console.log(
        `  ${transfer.type}: ${transfer.info.tokenAmount.uiAmount} ${transfer.info.mint} (${transfer.info.source} -> ${transfer.info.destination})`
      );
    }
