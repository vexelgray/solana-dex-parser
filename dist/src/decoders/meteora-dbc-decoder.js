...
exports.meteoraDBCDecoder = exports.MeteoraDBCDecoder = void 0;
...
const EVT_CREATE_CONFIG_DISCRIMINATOR = buffer_1.Buffer.from([131, 207, 180, 174, 180, 73, 165, 54]);
const EVT_CREATE_CONFIG_V2_DISCRIMINATOR = buffer_1.Buffer.from([163, 74, 66, 187, 119, 195, 26, 144]);
class MeteoraDBCDecoder extends idl_decoder_1.IdlDecoder {
    constructor() {
        super(meteora_dbc_json_1.default);
    }
    /**
     * Check if data is a Meteora DBC event (has Anchor event prefix)
     */
    isMeteoraDBCEvent(data) {
     ...
     */
    identifyEvent(data) {
        if (!this.isMeteoraDBCEvent(data))
            return null;
     ...
     */
    decodeEvtCurveComplete(data) {
        const buffer = buffer_1.Buffer.from(data);
        if (buffer.length < 16)
            return null;
        const eventName = this.identifyEvent(buffer);
        if (eventName !== 'EvtCurveComplete')
            return null;
        try {
            const eventData = buffer.subarray(16);
     ...
     */
    parseEvtCurveCompleteData(data) {
        let offset = 0;
        const pool = bs58_1.default.encode(data.subarray(offset, offset + 32));
        offset += 32;
        const config = bs58_1.default.encode(data.subarray(offset, offset + 32));
     ...
     */
    extractSqrtStartPriceFromCreateConfig(data) {
     ...
     */
    extractMigrationQuoteThresholdFromCreateConfig(data) {
        const result = this.extractConfigDataFromCreateConfig(data);
     ...
     */
    extractConfigDataFromCreateConfig(data) {
     ...
     */
    decodeEvtCreateConfig(data) {
        const buffer = buffer_1.Buffer.from(data);
        // Check for EvtCreateConfig event
        const eventName = this.identifyEvent(buffer);
        if (eventName !== 'EvtCreateConfig')
            return null;
        try {
            ...
            const feeClaimer = bs58_1.default.encode(eventData.subarray(64, 96));
            const owner = bs58_1.default.encode(eventData.subarray(96, 128));
     ...
     */
    decodeEvtCreateConfigV2(data) {
        const buffer = buffer_1.Buffer.from(data);
        // Check for EvtCreateConfigV2 event
        const eventName = this.identifyEvent(buffer);
        if (eventName !== 'EvtCreateConfigV2')
            return null;
        try {
            // Skip 16-byte Anchor event prefix
            const eventData = buffer.subarray(16);
    ...
    parseConfigDataV2(eventData) {
        try {
            ...
            for (let offset = 140; offset < Math.min(200, eventData.length - 24); offset++) {
                ...
                if (val >= 10000000000n && val <= 1000000000000n) {
     ...
     */
    parseConfigDataV1(eventData) {
        try {
            if (eventData.length < 180)
                return null;
            const config = bs58_1.default.encode(eventData.subarray(0, 32));
            const quoteMint = bs58_1.default.encode(eventData.subarray(32, 64));
     ...
     */
    bruteForceDecodeCreateConfig(data) {
...
exports.MeteoraDBCDecoder = MeteoraDBCDecoder;
// Export singleton instance for convenience
exports.meteoraDBCDecoder = new MeteoraDBCDecoder();
//# sourceMappingURL=meteora-dbc-decoder.js.map