...
class MeteoraDBCEventParser extends base_event_parser_1.BaseEventParser {
    ...
    getInstructionDiscriminators() {
        return {
            swap: decoders_1.meteoraDBCDecoder.getInstructionDiscriminator('swap'),
            swap2: decoders_1.meteoraDBCDecoder.getInstructionDiscriminator('swap2'),
            initSpl: decoders_1.meteoraDBCDecoder.getInstructionDiscriminator('initialize_virtual_pool_with_spl_token'),
            initToken2022: decoders_1.meteoraDBCDecoder.getInstructionDiscriminator('initialize_virtual_pool_with_token2022'),
            migrateDamm: decoders_1.meteoraDBCDecoder.getInstructionDiscriminator('migrate_meteora_damm'),
            migrateDammV2: decoders_1.meteoraDBCDecoder.getInstructionDiscriminator('migration_damm_v2'),
        };
    }
    initializeDiscriminators() {
        const discs = this.getInstructionDiscriminators();
        if (discs.swap)
            this.eventParsers.TRADE.discriminators.push(discs.swap);
        if (discs.swap2)
            this.eventParsers.TRADE.discriminators.push(discs.swap2);
        if (discs.initSpl)
            this.eventParsers.CREATE.discriminators.push(discs.initSpl);
        if (discs.initToken2022)
            this.eventParsers.CREATE.discriminators.push(discs.initToken2022);
        if (discs.migrateDamm)
            this.eventParsers.MICRATE.discriminators.push(discs.migrateDamm);
        if (discs.migrateDammV2)
            this.eventParsers.MICRATE_V2.discriminators.push(discs.migrateDammV2);
    }
    processEvents() {
            ...
            for (const group of this.adapter.innerInstructions || []) {
                for (const inner of group.instructions) {
                    try {
                        ...
                        if (data.length < 100)
                            continue; // EvtCreateConfig is large
                        const buffer = buffer_1.Buffer.from(data);
                        // Use brute force decoder that searches for discriminator at any offset
                        const configData = decoders_1.meteoraDBCDecoder.bruteForceDecodeCreateConfig(buffer);
                        if (configData) {
                            meteora_dbc_config_cache_1.MeteoraDBCConfigCache.set(configData.config, {
            ...
            try {
                ...
                for (const [eventType, parser] of Object.entries(this.eventParsers)) {
                    ...
                    if (parser.discriminators.some((it) => discriminator.equals(it))) {
                        ...
                        memeEvent.launchpad = constants_1.DEX_PROGRAMS.METEORA_DBC.name;
                        memeEvent.platform =
                            this.resolveLaunchpadPlatform(outerIndex, innerIndex) || constants_1.DEX_PROGRAMS.METEORA_DBC.name;
    ...
    decodeTradeEvent(data, options) {
        ...
        const tradeType = (0, utils_1.GetAccountTradeType)(new web3_js_1.PublicKey(options.signer), new web3_js_1.PublicKey(baseMint), new web3_js_1.PublicKey(inputTokenAccount), new web3_js_1.PublicKey(outputTokenAccount));
        if (tradeType == 'SELL') {
            inputMint = baseMint;
        ...
        if (swapData) {
            ...
            if (swapData.nextSqrtPrice) {
                ...
                const CURVE_TOKEN_SUPPLY = 1000000000000000n;
     ...
     */
    extractSwapDataFromEvent(outerIndex) {
        try {
            ...
            for (const inner of innerGroup.instructions) {
                try {
                    ...
                    if (data.length < 100)
                        continue;
                    const buffer = buffer_1.Buffer.from(data);
                    const eventName = decoders_1.meteoraDBCDecoder.identifyEvent(buffer);
    ...
    decodeCreateEvent(data, options) {
        ...
        const totalSupply = 1000000000000000; // 1B with 6 decimals (1e15)
        ...
        const cachedConfig = meteora_dbc_config_cache_1.MeteoraDBCConfigCache.get(configAddress);
        if (cachedConfig) {
            ...
            if (configData) {
                ...
                meteora_dbc_config_cache_1.MeteoraDBCConfigCache.set(configAddress, {
                    configAddress,
                    quoteMint,
        ...
        return {
            ...
            quoteToken: {
                mint: quoteMint,
                symbol: quoteMint === constants_1.TOKENS.SOL ? 'SOL' : 'QUOTE',
                decimals: 9,
            },
    ...
    extractConfigData(outerIndex) {
        try {
            ...
            for (let i = 0; i < outerIndex; i++) {
                ...
                if (!discriminator.equals(createConfigDisc))
                    continue;
                // Found create_config instruction, parse ConfigParameters
                const sqrtStartPrice = this.parseSqrtStartPriceFromConfigData(buffer_1.Buffer.from(data.slice(8)));
                if (sqrtStartPrice) {
     ...
     * may not have the standard Anchor self-CPI prefix
     */
    extractConfigDataFromCreateConfigEvent(outerIndex) {
        try {
     ...
     */
    parseSqrtStartPriceFromConfigData(data) {
        try {
            ...
            if (sqrtStartPrice === 0n || sqrtStartPrice > 1n << 128n)
                return undefined;
     ...
     */
    resolveRealCreator(instructionUser, launchpad) {
     ...
     */
    resolveLaunchpadPlatform(outerIndex, innerIndex) {
        // Strategy 1: Check signers (most launchpads)
        const signers = this.adapter.signers;
        for (const signer of signers) {
            const launchpad = constants_1.METEORA_DBC_LAUNCHPAD_SIGNERS[signer];
            if (launchpad) {
                return launchpad;
            }
        }
        ...
        if (innerIndex !== undefined) {
            ...
            if (outerInstruction) {
                ...
                const launchpad = constants_1.METEORA_DBC_LAUNCHPAD_PROGRAMS[callerProgramId];
                if (launchpad) {
                    return launchpad;
                }
            }
