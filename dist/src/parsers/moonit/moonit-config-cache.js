...
exports.MoonitConfigCache = void 0;
...
class MoonitConfigCacheClass {
    constructor() {
        ...
        this.DEFAULT_DECIMALS = 9;
        this.DEFAULT_MARKETCAP_THRESHOLD = BigInt('345000000000'); // 345 SOL
        this.DEFAULT_MIGRATION_FEE = BigInt('500000000'); // 0.5 SOL
        this.DEFAULT_COEF_B = 25;
    }
    /**
     * Store curve data in cache
     */
    set(curveAddress, input) {
     ...
     */
    async fetchFromRpc(connection, curveAddress) {
        // Check cache first
        const cached = this.get(curveAddress);
        if (cached)
            return cached;
        try {
            const accountInfo = await connection.getAccountInfo(new web3_js_1.PublicKey(curveAddress));
     ...
     */
    decodeCurveAccount(curveAddress, data) {
        try {
            const totalSupply = data.readBigUInt64LE(TOTAL_SUPPLY_OFFSET);
            const curveAmount = data.readBigUInt64LE(CURVE_AMOUNT_OFFSET);
            const mint = new web3_js_1.PublicKey(data.subarray(MINT_OFFSET, MINT_OFFSET + 32)).toBase58();
...
exports.MoonitConfigCache = new MoonitConfigCacheClass();
//# sourceMappingURL=moonit-config-cache.js.map